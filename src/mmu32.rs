...
const B_BIT: u32 = 1 << 2;
/// Bits[14:12] (TEX) = `0b001`, paired with C=1/B=1: Normal memory,
/// Outer and Inner Write-Back, Write-Allocate -- see the parent module's
/// doc comment on why RAM needs to be Cacheable, not just Shareable, for
/// `ldrex`/`strex` to actually succeed on this core.
const TEX_NORMAL_CACHEABLE: u32 = 0b001 << 12;
/// Bit[3] (C), paired with `TEX_NORMAL_CACHEABLE`/`B_BIT` above.
const C_BIT: u32 = 1 << 3;
...
const SECTION_RAM: u32 =
    DESCRIPTOR_SECTION | AP_FULL_ACCESS | TEX_NORMAL_CACHEABLE | C_BIT | B_BIT | S_BIT;
...
/// time (a plain `static`, not populated by any runtime loop). `TTBR0`
/// with `N=0` (a single top-level table, no split) requires 16KB
/// alignment -- `#[repr(align(16384))]` on a wrapper struct, since
/// `align` can't be attached directly to a `static`'s type when that type
/// is a plain array.
#[repr(align(16384))]
struct PageTable(
    // Never read through Rust field access -- only its address (via
    // `&PAGE_TABLE`) matters; the actual contents are read by the MMU's
    // hardware page-table walker, invisible to the compiler.
    #[allow(dead_code)] [u32; SECTION_COUNT],
);

static PAGE_TABLE: PageTable = PageTable(build_page_table());
...
/// single, already-built, read-only [`PAGE_TABLE`] -- never a second
/// build of the table itself.
///
/// # Safety
///
...
pub unsafe extern "C" fn rpi_hal_mmu_init() {
    let ttbr0 = &PAGE_TABLE as *const PageTable as u32;
